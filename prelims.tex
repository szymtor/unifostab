...
Mendez proved~\cite{nevsetvril2010first} that nowhere dense graph classes are exactly the same as uniformly quasi-wide classes. 



As mentioned in the introduction, Kreutzer, Rabinovich and the second author
 proved that for each radius $r$, we may always choose the function~$N$ to be a polynomial~\cite{siebertz2016polynomial}. 
%
...
%\end{theorem}
%
However, the proof of Kreutzer et al.~\cite{siebertz2016polynomial} relies on non-constructive arguments and does not yield explicit bounds on the degree of $N$. 

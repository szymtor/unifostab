...
% can be obtained by replacing all the parameters $\epsilon,\delta,\epsilon_1,\epsilon_2$ below by~$0$, and substituting the usage of \autoref{lem:closure-nd} with \autoref{lem:closure-be}.
%
...
%according to \autoref{lem:lower-nd}, there exists an integer $r$ 
...
%\end{lemma}
%
%For the second part of~\autoref{thm:vc-density-lower-bound}, we use the contrapositive of \autoref{lem:top-bnd-exp}. Since $\CCC$ has unbounded expansion, for some $r\in \N$ 
%we have that the value $|E(H)|/|V(H)|$ is unbounded among depth-$r$ topological minors $H$ of graphs from $\CCC$.
%By applying \autoref{lem:lower-be}, we find that for some $q\leq r$ the value

...
We are left with arguing the first and the last arrow, which both follow from simple rewriting arguments.
...
to every quantification $\exists_y$ in $\phi(\bar x)$ we add a guard $\delta(y,\bar s)$ stating that there is a path from some element of $\bar x$ to $y$ that has length at most $r$ and does not pass through
...
In essence, every quantification $\exists_y$ of a vertex $y$ in $G$ is replaced by quantification of $y$ in $G-S$ plus a disjunction over $s\in S$ of formulas where we assume $y=s$.
...
and, if applicable, also the set of vertices $S$. Such an effective variant of Gaifman's locality, \cref{lem:gaif}, was given by Grohe~\cite{}\todo{citation}, and it is not hard to see that the same can be done also
for the Feferman-Vaught lemma, \cref{lem:fv}. By examining our proofs, one can readily verify that all the stated determination relations can be made effective in this sense.

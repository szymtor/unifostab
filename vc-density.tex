...
We proceed with the proof of \cref{thm:vc-density}.
...
Let $B$ be an $r$-closure of $A$, given by~\cref{lem:closure-nd}.
  ...
  is at most $\Oof(|B|^{1+\delta})$, and each of these projections has size $\Oof(|B|^{\delta})$.
  	   Figure~\ref{fig:sketch} serves as  an illustration to the steps of the proof in the case $\ell=1$.\todo{Change $\phi,\epsilon,X,Y,Z$ in the figure.}
  	   \begin{figure}[h!]
  	   	\centering
...
We now move to the proof of \Cref{thm:vc-density-lower-bound},
...
according to \Cref{lem:lower-nd}, there exists an integer $r$ 
such that $\mathcal{G}_{r}\subseteq \CCC$. In particular, for every $n\in \N$ the $(r-1)$-subdivision $P^{r}(n)$ of graph $P(n)$ is contained in $\CCC$.
Now consider 
...
By applying \Cref{lem:lower-be}, we find that for some $q\leq r$ the value

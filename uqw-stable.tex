...
Again, let $G$ be a graph and  $S\subset V(G)$ a set of vertices.
...
defined so that $i$ and $j$ are in the same part if and only if $w_i=w_j$.
...
\begin{proof}
 ...
 Let $((c_1,\ldots,c_d),\cal P)$ be the local type of the tuples $\bar w_1$ and $\bar w_2$.
  ...
  Let $\bar y$ be a tuple of variables of length $|\cal P^S|$,
  and fix a bijection between the elements of $\bar y$ and the set $U^S$.
 	Define a tuple $\bar \alpha$ as follows. For $j=1..|x|$, if 
	$c_j\in S$, then let $\alpha_j=s$;
	otherwise, the $j$th element of $\bar \alpha$ is the 
	variable in the tuple $\bar y$ which corresponds to the part in the set $\cal P^S$ which contains the element $j$.
	For $i=1,2$, let $\bar w_i^S$ denote the tuple of length $|\bar y|$, which on position corresponding to the part $P\in \cal P^S$
	contains the vertex of $G$ which occurs in the tuple $\bar w_i$
	on all positions which belong to the part $P$. Note that by definition, $\bar w_i^S$ is a non-repeating tuple consisting of those vertices in $\bar w_i$ which do not belong to the set $S$. In particular, the elements of the tuple $\bar w_i^S$ form a $2r$-independent subset of $G-S$.

...
where
 $T$ is the number of all quantifier rank $q$ types with one variable for  graphs colored with $s^d(r)$ colors,
 and $N^d(r,m)$ and $s^d(r)$ are as in Proposition~\ref{prop:uqw-tuples}.
\end{theorem}
...
\begin{lemma}\label{lem:gaifman}
	...
	depends only on the quantifier rank $q$ type of  $(N^G_r(\bar a),\bar a)$,
 where $r=7^q$.
\end{lemma}
...
Finally, let $m=(T+s^d(r))^d+1$. Note that $m\le (T+s^d(r)+1)^d\le (2T)^d$.

We show that 
every $\phi$-ladder in a graph $G\in\cal C$ has length smaller than $N^d(r,m)$.
To reach a contradiction, assume that there is a graph $G\in\cal C$, a number $k\ge N^d(r,m)$
...
Applying Proposition~\ref{prop:uqw-tuples} to $A$, $r$ and $m$
		 yields a set $S\subset V(G)$ such that $|S|\le s^d(r)$
...
and otherwise, the color of $v$ is the quantifier rank $q$ type of  $(N^r_{G^S}(v),v)$.
...
which is proved by  playing the $q$-round Ehrenfeucht-Fraisse game. 
		...
		\begin{lemma}\label{lem:type-union}
			...
			have the same quantifier rank $q$ type.			
			Then the quantifier rank $q$ type of the disjoint union $(G_1\oplus H_1,\bar a_1\bar b_1)$ is equal to the one of $(G_2\oplus H_2,\bar a_2\bar b_2)$. \end{lemma}

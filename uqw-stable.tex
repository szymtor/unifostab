...
\begin{lemma}\label{lem:coloring}
	...
	vertices of $V(G)$ such that  $\bar u$ and $\bar v$ have the same $q$-local types. Then $$(G,\bar u)\models \phi(\bar x)\qquad \iff\qquad (G,\bar v)\models \phi(\bar x).$$
\end{lemma}
\begin{proof}
...
\begin{lemma}\label{lem:crossing}	Let $\phi(\bar x)$ be a formula with
  ...
  Suppose $\bar u\bar v$ and $\bar u'\bar v'$ have the same $q$-local type and $\set{\bar u\bar v,\bar u'\bar v'}$
...
\end{lemma}
\begin{proof}
Since $\bar u\bar v$ and $\bar u'\bar v'$ have the same $q$-local type, in particular $\alpha(\bar u\bar v)=\alpha(\bar u'\bar v')$.
This means that in tuples $\bar u\bar v$ and $\bar u'\bar v'$, the set of coordinates occupied by vertices of $S$ is the same, on moreover on each of these coordinates $\bar u\bar v$ and $\bar u'\bar v'$
feature the same vertex from $S$. It follows that $\alpha(\bar u\bar v')=\alpha(\bar u'\bar v)$, so we are left with verifying that the quantifier rank $q$ types of $N^r_{G^S}[(\bar u\bar  v')^S]$ and
$N^r_{G^S}[(\bar u'\bar  v)^S]$ are equal.

Observe that the tuple $(\bar u\bar v')^S$
...
of the tuples $\bar u^S$ and $(\bar v')^S$.\todo{Why??? The tuples do not need to be $r$-independent by themselves.}

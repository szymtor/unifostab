...
Recall that two tuples $\bar u,\bar v$ of vertices of $G$ are $r$-separated by $S\subset V(G)$ if every path from a vertex in $\bar u$ to a vertex in $\bar v$ of length at most $r$ passes through $S$.
...
$\uqw^d_r(N,s)$ if the following condition holds:
  ...
  whereas in the property $\uqw_r$, we may always assume that $S\subset B$. This distinction is inessential, since $|S|\le s_r$ for some constant $s_r$,

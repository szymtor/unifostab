...
a position $i$ such that $w_i\neq w_{i-1}$, where $w_0$ is assumed to be $\dau$.
 The \emph{alternation rank} of the tree $\tau$ is the maximal number of alternations, over of all words in the domain of $\tau$.


\begin{lemma}\label{lem:number-of-nodes}
	If $\tau$ has alternation rank at most $t$ and height at most $h$, then it has at most $h^{t+1}$
	nodes.
\end{lemma}
\begin{proof}		
	To each node $w$ of $\tau$ assign 
	the function $f_w:\set{1,\ldots,t+1}\to\set{1,\ldots,h}$ which
	...
	and if such an index $j$ does not exist, then $f_w(i)=|w|+1$.
		The mapping $w\mapsto f_w$ is injective, hence the domain of $\tau$ 
		has at most $h^{t+1}$ elements.	
\end{proof}
...
\begin{proof}
	...
	\end{claim}
	\begin{proof}
		If $i\le j$, then $d(a_i,b_j)\le 2$, but $d(a_i,b_j)>1$ since $A$

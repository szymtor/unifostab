...
\newcommand{\son}{\mathrm{S}}
  ...
  For $w\in \set{\dau,\son}^*$,
	...
	and $w$ is the \emph{parent} of both $w\son$ and $w\dau$.
	We consider
...
When processing the vertex $a$, do the following. Start with $w$ being the empty word. While $w$ is a node of $\tau$, repeat the following step: 
  ...
  $G$ is equal to $2$, replace $w$ by its son, otherwise, replace $w$ by its daughter.
...
\begin{proof}		
	...
	the function $f_w\colon \set{1,\ldots,t}\to\set{1,\ldots,h}$ defined as follows:
	$f_w$ maps each $i\in \set{1,\ldots,t}$ to the $i$th alternation of $w$, provided $i$ is at most the number of alternations of $w$, and otherwise we put $f_w(i)=|w|+1$.
	It is clear that the mapping $w\mapsto f_w$ is injective
...
\begin{proof}
	...
	\begin{clproof}
		...
		is $1$-independent. In particular, there is a vertex $z_{ij}$ which is a common neighbor of $a_i$ and $b_j$. 
...
To prove~\cref{lem:apex}, let $n_0$
be the number obtained from~\cref{lem:diversity}.
Fix integers $\ell\ge n_0$ and $m$, and define $h=m+\ell$.
Let $A$ be an $1$-independent subset of $G$
...
$Y$ be the set of all nodes $\tau(v)$ such that $v\son$ is a prefix of $w$. Then, by construction, $Y\subset A$ is a set of vertices which are mutually at distance exactly $2$ in $G$. By~\cref{cor:diversity}, there is a vertex $v\in G$
...
Fix an integer $t$ and a number $\beta>2\cdot t$. For $k\ge 0$,
define $m_k=((k+1)\cdot m)^{\beta^k}$.
...
\begin{corollary}\label{cor:apex}
  % Let $t$ be an integer and $\beta>2\cdot t$ a real.
	...
	$A\subset V(G)$ is an $1$-independent set which does not contain a $2$-independent set of size $m$ and $|A|\ge m_k$ %$((k+1)\cdot m)^{\beta ^k}$
	then there is a vertex $v$ of $G$ such that $|N_G(v)\cap A| \ge m_{k-1}$.%$(k\cdot m)^{\beta^{k-1}}$.
\end{corollary}
\begin{proof}
...
yielding a vertex $v$ with at least $\ell^\alpha=(k\cdot m)^{\beta^{k-1}}$ neighbors in~$A$.
\end{proof}

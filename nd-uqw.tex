...
We remark that the result of Kreutzer et al.~is sufficient to prove our main result, Theorem~\ref{thm:vc-density}. 
%, but is required in our proof of~Theorem~\ref{thm:new-stable}, which is the effective 
%variant of the result of Adler and Adler, Theorem~\ref{thm:new-stable}.
%
%
% \setcounter{theorem}{}
...
Finally, construct a graph $H$ on vertex set $A$ by making distinct $u,v\in A$ adjacent in $H$ whenever there is an edge in $G$ between %the branch sets~
$I_u$ and $I_v$.
...
By assumption, there is a vertex $w\in B$ that is adjacent both to $u$ and to $v$. Observe that if %it happens that 
$f(w)=u$ or $f(w)=v$, then $uv$ for sure becomes an edge in $H$. 
...
\newcommand{\son}{\mathrm{S}}
  ...
  For $w\in \set{\dau,\son}^*$,
	...
	We consider
	 ...
	 If $v$ is a node of $\tau$, then $\tau(v)$ its \emph{label}.
...
We defer the proof of the algorithmic statement of the lemma to the full version.
...
Theorem~\ref{thm:uqw-tuples} is an immediate consequence  of~Theorem~\ref{thm:new-uqw}
and of the following result.

\begin{lemma}\label{lem:uqw-tuples}
For all $r,d\in\N$,
...
The rest of Section~\ref{sec:uqw-tuples} is devoted to the proof of Lemma \ref{lem:uqw-tuples}.
Fix a class $\cal C$
such that $\uqw_{2r}(N_{2r},s_{2r})$ holds for some number $s_{2r}\in \N$ and  function $N_{2r}\from \N\to \N$.
We also fix the function $f$ defined in the statement of Lemma~\ref{lem:uqw-tuples}.
...
To finish the proof of Lemma~\ref{lem:uqw-tuples},
...
This concludes the proof of Lemma~\ref{lem:uqw-tuples}.

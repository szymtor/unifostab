...
For \marginpar{check numbers}every pair of integers $t,r\in \N$ there exists an integer $d<5r/2$ and a function $L\colon \N\to \N$ with $L(m)=\Oof_{r,t}{(m^{{(2t+1)}^{2rt}})}$ such that the following holds.
...
with at least $(m+\ell)^{2t}$ \marginpar{check numbers} elements,
...
\end{itemize}
Moreover, in each case, the corresponding structure (a depth-$4$ minor model, a $2$-independent set of size $m$, or a vertex $v$ as above) can be computed in time $\Oof_t(|A|\cdot |E(G)|)$. \marginpar{the depth-$4$ minor possibly not}
\end{lemma}
...
To prove \Cref{lem:apex} we need one more lemma. 
...
\begin{lemma}\label{lem:diversity}
  ...
  Then there exists a vertex $v$ in $G$ which has at least $|A|^{1/8}$ neighbors in $A$.
\end{lemma}
\begin{proof}
...
taking $L(m)$ as the above binomial coefficient yields~\cref{lem:engine} in case $r=0$, for $S=\emptyset$. Note here that $\binom{m+t-2}{t-1}\in\Oof_{t}{(m^{{(2t+1)}^{2t}})}$.\marginpar{check numbers}
...
Fix a number $\beta>2t$. For $k\ge 0$,\marginpar{check numbers}
...
\end{claim}
\begin{clproof}
Let $\alpha=t/\beta$, so that $\alpha<1/2$.\marginpar{check numbers}
...
if $|A|=|A'|\ge {m+t-2\choose t-1}$, then
 ...
 Hence, the obtained bound is $L(m)={m+t-2\choose t-1}$, and we have already argued that then $L(m)\in \Oof_{r,t}{(m^{{(2t+1)}^{2t}})}$.\marginpar{check numbers}
...
Fix \marginpar{check numbers} integers $r,t$,  and a graph $G$ such that $K_t\not\minor_{d} G$,

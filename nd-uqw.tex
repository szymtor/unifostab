...
\begin{corollary}\label{cor:apex}
	...
	$A\subset V(G)$ is an independent set which does not contain a $2$-independent set of size $m$, and $|A|\ge (k\cdot m)^{\beta ^k}$
	then there is a vertex $v$ of $G$ such that $|N_G(v)\cap A| \ge ((k-1)\cdot m)^{\beta^{k-1}}$.
\end{corollary}
...
Assume that $|A|\ge ((t+1)\cdot \beta)^{\beta^{t+1}}$. By induction, we
 construct a sequence  $A=A_0\supseteq A_1\supseteq\ldots$ 
of $1$-independent subsets of $G$
of length at most $t$,
such that $|A_i|\ge ((t+1-i)\cdot\beta)^{\beta^{t+1-i}}$,
...
If $A_{i-1}$ contains a $2$-independent set of size $m$ in $G-S$, terminate.
 ...
 $((t+1-i)\cdot\beta)^{\beta^{t+1-i}}$ vertices of $A_{i-1}$.
 Let $A_{i}$ consist of those vertices, and add $v_i$
 to the set $S$. Proceed by replacing $i$ by $i+1$.
 ...
 and $|A_i|\ge \beta^\beta\ge t^2$ for $i\le t$. Hence,
if $v_1,\ldots,v_i$ and $A_i$ are constructed, 
then $G$ contains the complete bipartite graph with $i$ vertices in one part and $t^2$ vertices in the other part, and, in particular, $K_i\minor_2 G$.
 Since $K_t\not\minor_2 G$, the process must terminate with $i<t$.

...
taking $L(m)$ as the above binomial coefficient yields~\cref{lem:engine} in case $r=0$, for $S=\emptyset$.
...
We may choose pairwise distinct vertices $w_1,\ldots,w_k$,
 ...
 Therefore, at some step $k<t$ of the construction we must have obtained a $2$-independent subset of $G-S$ of size $m$. Moreover, $|S|\le k<t$.
 
 
 
 This proves~\cref{lem:engine} in the case $r=2$, for $d=2$, and for the function $L=L_t$, defined by $L_t(m)=(m+t\cdot m)^{\beta^t}$
...
We now wrap up the proof of \cref{thm:new-uqw} by applying~\cref{lem:engine} by induction on $r$.
...
define the function $L_t(m)=((t+1)\cdot m)^\gamma$.
...
Observe that by construction, $|S_r|\le r\cdot t/2$, as in the odd steps, the constructed set $S$ is empty, and in the even steps, it has at most $t$ elements.  We claim that in fact we have $|S_r|<t$. The argument is similar as in the proof of \cref{claim:at-most-t}.
...
This proves~\cref{thm:new-uqw}, where as the function $N$ we may take the following function
$$N(m)=(m(t+1))^{{(2(t+1))}^{r(t+1)}}.$$
\end{proof}

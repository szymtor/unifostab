...
For \marginpar{check numbers}every integer $t\in \N$ and positive real $\alpha<\frac 1 2$ 
...
\begin{proof}Let ${\cal F}=\set{N(v)\cap A\colon v\in V(G)}$. 
  ...
  Hence $\frac{\binom{|A|}{2}}{\binom{k}{2}}\leq |{\cal F}|\le  |A|^{1+1/3}$, implying $k\ge |A|^{1/3}$.\marginpar{this is not entirely correct}
\end{proof}

...
Define the function  $K:\N\to\N$
by $K(m)=(t+1)^{\beta^{t(r+1)}}\cdot m^{\beta^{tr}}$
for $m\ge m_\bullet$  and $K(m)=m_\bullet$.
The argument above shows that if $|A|\ge K(m)$ then 
there is a set $S\subset V(G)$, equal to $S_r$ above,
and a set $B\subset A$, equal to $A_r$ above,
so that $B$ is $r$-independent in $G-S$. Note that if $\beta=2t+1$ and $d=N(m_\bullet)$, then $K(m)\le N(m)$,
where $N$ is the function in the statement of~\cref{thm:new-uqw}.
% This proves the

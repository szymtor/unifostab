...
$\binom{t+1}{2}$ vertices, we can take $c_{K_t}=\binom{t+1}{2}$ and
...
Choose any pairwise distinct vertices $w_1,\ldots,w_k\in A_k$ and observe that the connected subgraphs $\set{w_i,v_i}$ of~$G$ yield a depth-$1$ minor model of $K_k$ in $G$.
 ...
 It is easy to see that then $L_t(m)=L(m)\in \Oof_{t}{(m^{{(4t+1)}^{2t}})}$, provided we put $\beta=4t+1$.
...
for $d=\lfloor 9r/2 \rfloor$. Let $\beta>4t$ be a fixed real. As in the proof of \cref{lem:engine}, without loss of generality suppose $m\geq t^8$.
...
Note that $N(m)\in \Oof_{r,t}{(m^{{(4t+1)}^{2rt}})}$, provided we put $\beta=4t+1$.

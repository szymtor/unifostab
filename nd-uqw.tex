...
For \marginpar{check depth}every pair of integers $t,r\in \N$ there exists an integer $d<5r/2$ and a function $L\colon \N\to \N$ with $L(m)=\Oof_{r,t}{(m^{{(8t+1)}^{2rt}})}$ such that the following holds.
...
if $|A|=|A'|\ge {m+t-2\choose t-1}$, then
 ...
 \paragraph{Even case.}
 Finally,
 we prove~\cref{lem:engine} in the case $r=2s+2$, for some integer $s\geq 1$. We put $d=5s+2=5r/2-3$. \marginpar{check depth}
Let $G$  be such that 
 $K_t\not\minor_{d} G$, and

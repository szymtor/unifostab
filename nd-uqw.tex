...
for all $G\in \CCC$, where $K_{t(r)}$ denotes the clique on $t(r)$ vertices. \textcolor{red}{The class $\CCC$ has \emph{bounded expansion}
...
\marginpar{This lemma is wrong, we need that $G$ comes from a nowhere dense class if we want it to work for every $\epsilon$}Let $G$ be a graph such that $K_t\not\minor_{1} G$ for some constant $t\in \N$. 
...
\begin{proof}Let ${\cal F}=\set{N(v)\cap A\colon v\in V(G)}$. 
  ...
  \marginpar{$k\ge \frac{1}{2}|A|^{(1-\epsilon)/2}$}
\end{proof}

...
For a nonnegative integer $s$, by $N_s[u]$ we denote the {\em{(closed) $s$-neighborhood}} of $u$ which comprises vertices at distance at most $s$ from $u$; note that $u$ is always contained in its closed $s$-neighborhood. The \emph{radius} of a graph $G$ is the least integer $r$ such that there is some vertex $v$ of $G$ with $N_r[v]=V(G)$.
...
Our proof follows exactly the same lines as the original proof of Ne\v set\v ril and Ossona de Mendez, with the difference that in the key technical lemma (\cref{lem:apex} below), 
we improve the bounds significantly by replacing a Ramsey argument with a new combinatorial reasoning.
The new argument essentially originates in the concept of {\em{branching index}} from the stability theory, 
...
For all $t,r\in \N$ there exists a function $L\colon \N\to \N$ with $L(m)=\Oof_{r,t}{(m^{{(2t+1)}^{r(t+1)}})}$ such that the following holds.
...
in place of the bound $(m+\ell)^t$ a bound of the form $R(m,\underbrace{q,q,\ldots,q}_{k\text{ times}})$,
...
Note that whenever $i\leq j$ and $i'\leq j'$ are such that $j\neq j'$, vertices $z_{ij}$ and $z_{i'j'}$ are different, because $z_{ij}$ is adjacent to $b_{j}$ but not to $b_{j'}$, and the converse holds fro $z_{i'j'}$.
...
\paragraph{Case $r=1$.}
Note that in the statement we assume that $K_t\not\minor_2 G$, but we will actually prove a stronger statement where we only assume that $K_t\not\minor_0 G$; 
that is, $G$ does not contain a clique of size $t$. By Ramsey's Theorem, in any graph every set of size $\binom{m+t-2}{t-1}$ contains an
independent set of size $m$ or a clique of size $t$. Therefore, 
taking $L(m)$ as the above binomial coefficient yields~\cref{lem:engine} in case $r=0$, for $S=\emptyset$. Note here that $\binom{m+t-2}{t-1}=\Oof_{t}{(m^{{(2t+1)}^{t+1}})}$.
...
\paragraph{Case $r=2$.}
Note that in the statement we assume that $K_t\not\minor_5 G$, but we will actually prove a stronger statement where we only assume that $K_t\not\minor_2 G$.
...
\begin{corollary}\label{cor:apex}
  % Let $t$ be an integer and $\beta>2\cdot t$ a real.
	There is an integer $\mbull$ such that for all $m\ge \mbull$ and $k\ge 1$, the following holds.
...
Let $\alpha=t/\beta$, so that $\alpha<1/2$, 
 and let 
$\ell=(k\cdot m)^{(2\beta)^{k-1}/\alpha}$.
Take $\mbull= \ell_0^{t/\beta}$, where $\ell_0$ is the constant given by \cref{lem:apex} for $t$ and $\alpha$.
...
we may  apply \cref{lem:apex},
yielding a vertex $v$ with at least $\ell^\alpha=(k\cdot m)^{(2\beta)^{k-1}}$ neighbors in~$A$.
\end{proof}

% \begin{proof}  [of~\cref{lem:engine}]

Let $G,t$ be such that $K_t\not\minor_2 G$, and fix some number $\beta>2\cdot t$. Let $\mbull$ be the number granted by~\cref{cor:apex}. If $m<\mbull$, replace $m$ by $\mbull$. We will find 
...
If $A_{i}$ contains a $2$-independent set of size $m$ in $G-S$, terminate.
 Otherwise, 
 apply~\cref{cor:apex} to the graph $G-S$ with $1$-independent subset
...
We may choose pairwise distinct vertices $w_1,\ldots,w_k$,
 ...
 This proves~\cref{lem:engine} in the case $r=2$, for $d=2$, and for the function $L=L_t$, defined by $L_t(m)=m_t=((t+1)\cdot m)^{\beta^t}$
 for $m\ge \mbull$ and $L(m)=L(\mbull)$ for $m<\mbull$, where $\beta>2\cdot t$ is any fixed constant and $\mbull$ is obtained from~\cref{cor:apex}.
 The proof yields an algorithm constructing the sets $A'$ and $S$,
 which amounts to applying at most $t$ times the procedure obtained in~\cref{lem:apex}.
...
for $d=\lceil 5r/2 \rceil$. Let $\beta>2\cdot t$ be a fixed real, and let $\mbull$ be the number from~\cref{cor:apex}. Without loss of generality suppose $m\geq t$ and $m\ge \mbull$.
...
for $m\ge \mbull$  and $N(m)=N(\mbull)$ for $m<\mbull$.
...
we get $N(m)=\Oof_{r,t}{((m(t+1))^{{(2t+1)}^{r(t+1)}})}.$
% This proves the

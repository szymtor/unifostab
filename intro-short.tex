...
For all $r,t\in \N$ there exists a function $N\colon \N\to \N$ with $N(m)\in \Oof_{r,t}(m^{(6t+3)^{r}})$ such that the following holds.
...
and  $G$ is a graph such that $K_t\not\minor_{g(q)} G$, then the ladder index of $\phi$ on $G$ is at most $f(q,d,t)$. \marginpar{with any division of the variables}
\end{theorem}

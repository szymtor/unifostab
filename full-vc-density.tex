...
\item If\, $\CCC$ has bounded expansion, then there exists a
 ...
 \medskip The proof of~Theorem~\ref{thm:vc-density} spans the
...
We proceed with the proof of Theorem~\ref{thm:vc-density}.  To focus
...
Let $q$ be the quantifier rank of $\phi$ and let $p,r$ be the numbers
...
%
%
% A graph $H$ is a \emph{topological depth-$r$ minor} of~$G$ if there
% is a mapping $\phi$ that maps vertices of~$H$ to vertices of $G$
% such that $\phi(u)\neq \phi(v)$ for $u\neq v$, and edges of $H$ to
% paths in $G$ such that if $uv\in E(H)$, then $\phi(uv)$ is a path of
% length at most $2r+1$ between $u$ and $v$ in $G$ and furthermore, if
% $uv, xy\in E(H)$, then $\phi(uv)$ and $\phi(xy)$ are internally
% vertex disjoint. We write $H\minor_r^{\mathrm{t}} G$.  Note that the
% above definition makes sense for half-integers, i.e., numbers $r$
% for which $2r$ is an integer.
%
% It is well-known that classes of bounded expansion can be
% alternatively characterized by the sparsity of shallow topological
% minors.
%
%\begin{lemma}[Corollary 4.1 of \cite{sparsity}]\label{lem:top-bnd-exp}
%  A class $\CCC$ of graphs has bounded expansion if and only if for
%  every $r\in \N$ there exists a constant $c_r$ such that
%  $|E(H)|/|V(H)|\leq c_r$ for all graphs $H$ such that
%  $H\minor_r^{\mathrm{t}} G $ for some $G\in \CCC$.
%\end{lemma}
%
...
% We will need the following result relating edge density in shallow
% topological minors and normed neighborhood complexity.

%\begin{lemma}[Theorem 1 of \cite{reidl2016characterising}]\label{lem:lower-be}
%  Let $G$ be a graph, $r$ be a half-integer, and let
%  $H\minor_r^{\mathrm{t}}G$.  Then
%\begin{align*}

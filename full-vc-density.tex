...
\medskip
We now move to the  second part of~Theorem~\ref{thm:vc-density-lower-bound}.
...
For the second part of~Theorem~\ref{thm:vc-density-lower-bound}, we use the contrapositive of Lemma~\ref{lem:top-bnd-exp}. Since $\CCC$ has unbounded expansion, for some $r\in \N$ 
we have that the value $|E(H)|/|V(H)|$ is unbounded among depth-$r$ topological minors $H$ of graphs from $\CCC$.
By applying Lemma~\ref{lem:lower-be}, we find that for some $q\leq r$ the value

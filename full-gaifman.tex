...
%This coincides with the standard notion of the first-order type of quantifier rank $q$ of the tuple $\bar u$.
...
For a fixed formula $\phi(\bar x,\bar y)$,  graph $G=(V,E)$ and sets $A,W\subset V$, define
...
\begin{lemma}[Gaifman locality,~\cite{gaifman1982local}]\label{lem:gaif}
  ...
  colors. %, and $A\subset V$ be a set of vertices of $G$.
...
The colored graph $G^S$ is called a \emph{plebeian companion} of 
$G$ in~\cite{AjtaiG94}. 
...
The argument for the last arrow is provided by the following lemma. 
Its proof can be found in~\cite{AjtaiG94} or more explicitly in~\cite{dawar2010homomorphism}. The proof in~\cite{dawar2010homomorphism} is constructive and one directly
derives that the formula~$\phi^S$ is computable. 

\begin{lemma}\label{cl:rewrite}
...
Lemma~\ref{cl:rewrite}, applied to $k=|\bar u\bar w|$, implies that 

...
if there is a bound $d\in\N$ such that the VC-dimension of $\phi(\bar x,\bar y)$ on $\str A$ is at most $d$ for all $\str A\in\cal C$.  \todo{This definition can only be right for classes which are closed under substructures.}
...
by providing essentially the optimum exponent.\todo{can we bring the result to one page?}
...
Finally, in \cref{sec:stable} we provide a constructive proof of the result of Adler and Adler, \cref{thm:new-stable}.\todo{page break here?}

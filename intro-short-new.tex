...
\textcolor{red}{One of the main contributions of \cite{eickmeyer2016neighborhood}
...
Distance neighborhood complexity is a 
...
$\strA$ is the set
\[\{\{\tup a\ \in A^m : \strA\models\psi(\tup a,b)\} : \tup b\in V(\strA)\}.\]
The number of types that can be realized over a parameter
...
\noindent Note that in particular, \cref{thm:new-stable} implies that every nowhere dense graph is stable, which was the main conclusion of Adler and Adler~\cite{adler2014interpreting}.

\textcolor{red}{Finally, we prove the following theorem concerning the number of
types in nowhere dense graph classes. }

\textcolor{red}{\begin{theorem}\label{thm:vc-density}
...
\textcolor{red}{The proof of the theorem can easily be modified to work for classes 
of bounded expansion.}

\textcolor{red}{\begin{theorem}\label{thm:vc-density-exp}
Let $\CCC$ be a class of bounded expansion
and let $\psi(\tup x,y)$ be a first-order formula, where 
$\tup x$ is an $m$-tuple of variables and $y$ is a single variable. 
There exists a constant~$c$ such that for every $G\in \CCC$ 
and every
$A\subseteq V(G)$, we have  $|S_\psi(A,G)|\leq c\cdot |A|$.
\end{theorem}}

\textcolor{red}{Also, it is not difficult to see that our result in fact yields a 
characterization 
of nowhere dense graph classes which are closed under taking 
subgraphs. }

\textcolor{red}{\begin{theorem}\label{thm:vc-density-lower-bound}
Let $\CCC$ be a somewhere dense class of graphs which 
is closed under taking subgraphs. Then there is a formula 
$\psi(x,y)$ such that there are $G\in\CCC$ and $A\subseteq V(G)$ 
with $|S_\psi(A,G)|=2^{|A|}$. 
\end{theorem}}

\textcolor{red}{The proof of \Cref{thm:vc-density} is (again) based on Gaifman's locality
theorem for first-order logic, which allows us to focus at local 
neighborhoods around the elements $v\in V(G)$. 
Using the closure lemma of~\cite{eickmeyer2016neighborhood}
for nowhere dense graph classes and uniform quasi-wideness
we are then able to well control and classify 
the interaction of tuples
$v$ with the set $A$. }

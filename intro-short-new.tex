...
shows that for stable classes of structures this does not happen.
...
A class of structures $\cal C$
   ...
   models of the theory of $\cal C$.} of~$\cal C$, and for all $B\subset V(\str A)$:
...
Shelah's result provides an upper bound on the number of types, albeit using infinite cardinals, elementary limits, and infinite parameter sets.
 ...
 however, does not seem to  immediately translate to a result of finitary nature for classes of finite graphs. As we describe below,
...
such that  $$\strA\models\phi(\tup{u}_i,\tup{v}_J)\Longleftrightarrow i\in J\qquad \text {for all $i\in I,J\subset I$.}$$
